...
///
/// `Pipeline` is itself an `AsyncSequence`. Each iteration reconstructs the underlying
...
public struct Pipeline<Success: Sendable, Failure: Error & Sendable>: AsyncSequence, Sendable {
    ...
    static func erased(_ build: @escaping @Sendable () -> AnyAsyncSequence<Element>) -> Pipeline {
...
extension Pipeline where Failure == Never {
    ...
    func widenFailure<F: Error & Sendable>(to _: F.Type) -> Pipeline<Success, F> {
